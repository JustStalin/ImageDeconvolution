...
OUT_DIR = Path("outputs/figures/wiener_showcases")
...
def collect_cases() -> tuple[pd.DataFrame, dict[str, dict[str, object]]]:
    ...
    df = df[df["best_method"] == "wiener"].sort_values(
...
def save_case_figure(case_id: str, payload: dict[str, object], rank: int) -> None:
    ...
    for ax, (title, image) in zip(axes, panels):
        ...
        ax.axis("off")
    fig.suptitle(
        f"Wiener showcase: {case.image.name}, {kernel.kernel_id}, sigma={case.noise_sigma:.2f}",
        fontsize=17,
    )
    fig.tight_layout()
...
def main() -> None:
    ...
    df.to_csv(OUT_DIR / "wiener_showcases.csv", index=False, encoding="utf-8-sig")
